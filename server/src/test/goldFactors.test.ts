...
import { alignFactors, parseXauJsonp, parseEmKlines } from '../data/goldFactors.js';
...
function candleWithFactor(date: string, close: number, xau: number, cnh: number, dxy: number): Candle {
  const premium = close - (xau * cnh) / 31.1035;
  return { date, open: close, high: close + 1, low: close - 1, close, volume: 1000, xau, cnh, dxy, premium };
}
